...
class FuzzySystemBoid: #está muito arteficial
    ...
    def __setup_membership_functions(self):
        ...
        # Membros do Predador ,PQ ISTO N ESTA NO PREDADOR?
        ...
        for c in [self.Forca_Separacao, self.Forca_Coesao, self.Forca_Alinhamento, self.Forca_Evasao]: #Tirei daqui a parte da evasão
            c['fraca'] = fuzz.trimf(c.universe, [0, 0, 4])
            c['media'] = fuzz.trimf(c.universe, [3, 5, 7])
            c['forte'] = fuzz.trimf(c.universe, [6, 10, 10])
            c['muito_forte'] = fuzz.trimf(c.universe, [9, 10, 10])
